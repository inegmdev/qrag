...
- [x] **GH#41** [EXPLORE-A] `cli.py`, `database.py` — MVP: `qrag explore list` (local Rich table) + `qrag explore stats <version>` (language %, symbol taxonomy, keyword tag cloud, staleness, coverage). Fixed in feat/gh41-explore-list-stats: added `get_code_stats()`/`get_docs_stats()` to database.py; `explore list` shows active marker ★, sizes, build date; `explore stats` renders summary/language/symbol/keyword-cloud panels; `hub` group made hidden with deprecation warning. [GitHub](https://github.com/inegmdev/qrag/issues/41)
