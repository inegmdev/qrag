...
- [ ] **AD-17** `mcp_server.py` — Session-scoped database selection: add `list_databases`, `set_active_databases`, `reset_active_databases` MCP tools so the LLM can narrow which globally active DBs are searched per conversation (in-memory, per-process, no session-ID needed under stdio transport) instead of fanning out across the full global `active_versions` set every query. Includes server-tracked `_session_scoped` first-use gating (`scope_hint`) and an `excluded_active_dbs` field for LLM-reasoned fallback suggestions. Design finalized via `/grilling`; see `docs/ARCHITECTURE.md` AD-17.
