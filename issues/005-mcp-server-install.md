...
- [ ] `raghub mcp install --ai=gemini` writes the correct Gemini CLI MCP config entry pointing to the installed `raghub` package entry point
- [ ] `raghub mcp install --ai=claude` writes the correct Claude MCP config entry
- [ ] `raghub mcp active <version>` sets `active_version` in `~/.raghub/config.json`
- [ ] `raghub mcp status` shows the active version, the path to both DBs, and whether the MCP server entry is registered in the detected AI tool config
- [ ] `raghub mcp info` prints the active version's `config.json` metadata (SoC, SDK version, TRM version, embedding model)
