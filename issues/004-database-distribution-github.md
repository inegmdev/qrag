...
- [ ] `quickrag-ti push <version> --repo github` uploads `code.db`, `docs.db`, `config.json`, and `manifest.json` as assets to a GitHub Release tagged `<version>`
- [ ] `quickrag-ti list-databases --repo github` lists all available release tags with their SoC and SDK version metadata
- [ ] `quickrag-ti download <version>` fetches the release assets into `~/.quickrag-ti/<version>/` with a progress bar; completes in under 60 seconds for a ≤500 MB package on a typical office connection
- [ ] `quickrag-ti delete <version>` removes the local `~/.quickrag-ti/<version>/` directory after confirmation prompt
