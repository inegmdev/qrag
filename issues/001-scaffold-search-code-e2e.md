...
Bootstrap the Python package and wire up the full embed→store→query pipeline using a hardcoded toy C fixture (no tree-sitter yet). By the end of this slice, `quickrag-ti search-code "query"` must return ranked results from a small in-repo fixture file.
...
- [ ] `pyproject.toml` defines the package with a `quickrag-ti` console script entry point
...
- [ ] `quickrag-ti search-code "enable error correction"` returns at least one result with `symbol_name`, `file_path`, `line_start`, `line_end`, `similarity_score`
