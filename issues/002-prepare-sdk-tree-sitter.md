...
Implement the `raghub prepare --soc <SOC> --sdk <path> --output <version>` command for C/C++ source trees. Tree-sitter extracts function and struct boundaries; the chunker splits oversized functions with a token-count fallback. Results land in `~/.raghub/<version>/code.db` with the symbols table populated for exact-lookup queries.
...
- [ ] `raghub search-code "initialize DMA channel"` returns relevant results from the real SDK
- [ ] `raghub get-symbol <name>` (debug CLI alias for `get_symbol_definition`) prints the full function body
