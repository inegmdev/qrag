...
Extend `raghub prepare` with a `--docs <path>` flag that ingests TRM PDFs, datasheet PDFs, and HTML pages into `~/.raghub/<version>/docs.db`. Sections are chunked at the heading level, preserving chapter/section hierarchy and page references. The `search-trm` CLI command queries this database.
...
- [ ] `raghub search-trm "ECC SRAM configuration"` returns the correct TRM section with page reference
